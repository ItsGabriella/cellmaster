...
function filtrarEstoque($buscaE = "", $categoria = "Todas", $status = "Todos") {
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        foreach ($result as $coluna) {
            ...
            <div class="modal fade" id="modalExcluir'.$coluna["idpeca"].'" tabindex="-1" aria-hidden="true">
              <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content border-0 shadow">
                  ...
                  <div class="modal-body text-center py-4">
                    ...
                    <p class="text-muted small mb-0">Você está prestes a remover a peça <strong>'.htmlspecialchars($coluna["nome_peca"]).'</strong>. Esta ação não poderá ser desfeita.</p>
                  </div>
                  <div class="modal-footer bg-light border-0 justify-content-center">
                    <button type="button" class="btn btn-outline-secondary px-4" data-bs-dismiss="modal">Cancelar</button>
                    <a href="php/salvarEstoque.php?codigo='.$coluna["idpeca"].'&funcao=D" class="btn btn-danger px-4 fw-semibold">Sim, Excluir</a>
                  </div>
                </div>
              </div>
            </div>
